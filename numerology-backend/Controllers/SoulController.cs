...
namespace numerology_backend.Controllers
{
    ...
    {
        private ISoulService _service;

        public SoulController(ISoulService service)
        {
            _service = service;
        }
        ...
        [HttpPost("soulsave")]
        public async Task<IActionResult> soulsave(SoulRequest model)
        {

            //try
            //{
            //  var data = _service.PlanetMasterInsertUpdate(model);
            var response = await _service.SoulMasterInsertUpdate(model);
            if (response == null)
        ...
        [HttpGet("soulGET")]
        public async Task<IActionResult> SoulGET(string startIndex, string pageSize, string searchBy, string searchCriteria, string tableDataJsons)
        {
            try
            {
                ...
                DataTable response = await _service.SoulGET(model, idString);
